...
test.describe("Collaboration E2E Tests", () => {
	// biome-ignore lint/suspicious/noExplicitAny: Test process variable needs flexible typing
	...
	test("should handle page refresh gracefully", async ({ page }) => {
		...
		await expect(counterDisplay).toContainText("0");
		await expect(
			page.locator("[data-testid='todo-list']").locator("li"),
		).toHaveCount(0);
	});
	...
	test("should handle edge cases", async ({ page }) => {
		...
		await expect(page.locator("[data-testid='message-display']")).toContainText(
			longMessage,
		);
		...
		await expect(page.locator("[data-testid='todo-list']")).toContainText(
			specialTodo,
		);
	});
});
